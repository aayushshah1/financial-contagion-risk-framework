...
KPCA_N_COMPONENTS = 3
KPCA_KERNEL = "rbf"
KPCA_GAMMA = None  # Auto: 1 / (n_features * X.var()) — set after seeing data
...
def build_danger_vector(features: List[str],
    ...
    Design principle (Option A — population-mean anchor):
      ...
      For all other CAMELS features (CDR, NIM, cost metrics, ROA, etc.):
        → Use the POPULATION MEAN of the transformed health scores.
          Rationale: "a bank exactly at the regulatory trigger, with otherwise
          average behaviour across all supervisory dimensions."
          This cleanly isolates regulatory distance without contaminating
          it with heuristic benchmarks the RBI does not use for PCA.
    ...
    """
    # Pre-compute population means for non-PCA fallback
    pop_mean = X_health.mean(axis=0)  # shape (n_features,)
    ...
    for j, feat in enumerate(features):
        ...
        if is_pca_pillar:
            ...
            if danger_val is not None:
                ...
                danger[j] = pop_mean[j]  # fallback to population mean
        else:
            # Non-PCA feature: anchor to population mean
            danger[j] = pop_mean[j]
...
def fit_kpca(X: np.ndarray,
    ...
    if gamma is None:
        # Heuristic: 1 / (n_features * X.var()) — bandwidth matches feature spread
...
def orient_manifold_with_merton(Z: np.ndarray,
    ...
    # Predict DTD rank for ALL banks (including those without Merton data)
    dtd_rank_predicted = ridge.predict(Z)

    # Invert: high DTD rank (low stress) → low stress score
    # We want stress_axis: high = most stressed (lowest DTD rank = rank 1)
    # So stress_raw ~ max_rank - predicted_rank (reverse the rank)
    max_rank = float(len(bank_names))
    stress_raw = max_rank - dtd_rank_predicted

    # Normalise to [0,1]
    mn, mx = stress_raw.min(), stress_raw.max()
    stress_axis = (stress_raw - mn) / (mx - mn + 1e-10)

    print(f"  Ridge R² on Merton coverage subset: "
          f"{ridge.score(Z_aligned, rank_aligned):.4f}")
...
def run_stress_analysis(year: str = "2025",
    ...
    for i, bank in enumerate(bank_names):
        merton_dtd = None
        merton_rank = None
        if merton_data and bank in merton_data:
            bd = merton_data[bank].get("merton", {})
            merton_dtd = bd.get("distanceToDefault")
            merton_rank = merton_data[bank].get("dtdRank")
