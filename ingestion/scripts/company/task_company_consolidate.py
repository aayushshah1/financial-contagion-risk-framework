...
SHP_DIR         = _REPO_ROOT / "data" / "company" / "shareholding"
...
def _shp_extract_arelle(model) -> Dict[str, Any]:
    ...
    for fact in model.facts:
        ...
        if detail_axes:
            # Entity row — store under EVERY Details axis found
...
# Long-term rating → one-year cumulative default rate (CDR)
# Source: CRISIL FY15-25 one-year CDR study (Table 1, Monthly static pools)
# Anchor grades (exact): AAA=0.00%, AA=0.05%, A=0.07%, BBB=0.46%, BB=2.86%, B=8.40%, C=24.98%
# Sub-grades are linearly interpolated within each category band.
_LT_STRESS: Dict[str, float] = {
    "AAA":  0.00000,  # 0.00% — anchor
    "AA+":  0.00025,  # midpoint AAA→AA
    "AA":   0.00050,  # 0.05% — anchor
    "AA-":  0.00057,  # AA→A, 1/3 step
    "A+":   0.00063,  # AA→A, 2/3 step
    "A":    0.00070,  # 0.07% — anchor
    "A-":   0.00200,  # A→BBB, 1/3 step
    "BBB+": 0.00330,  # A→BBB, 2/3 step
    "BBB":  0.00460,  # 0.46% — anchor
    "BBB-": 0.01260,  # BBB→BB, 1/3 step
    "BB+":  0.02060,  # BBB→BB, 2/3 step
    "BB":   0.02860,  # 2.86% — anchor
    "BB-":  0.04707,  # BB→B, 1/3 step
    "B+":   0.06553,  # BB→B, 2/3 step
    "B":    0.08400,  # 8.40% — anchor
    "B-":   0.16690,  # B→C, midpoint
    "C":    0.24980,  # 24.98% — anchor
    "D":    1.00000,  # 100% by definition
}

# Short-term rating → CDR via long-term correspondence
# A1+=AAA, A1=AA, A2+=A+, A2=A-, A3+=BBB, A3=BBB-, A4+=BB, A4=B
_ST_STRESS: Dict[str, float] = {
    "A1+": 0.00000,  # ≈ AAA
    "A1":  0.00050,  # ≈ AA
    "A2+": 0.00063,  # ≈ A+
    "A2":  0.00200,  # ≈ A-
...
# Ratings that carry no scoreable information
_SKIP_BASES = {"withdrawn", "not applicable", "issuer not cooperating",
               "rating outstanding with outlook", "na", ""}


def _parse_crisil_rating(raw: str) -> tuple:
    """
    Parse a CRISIL rating string into (base_stress, multiplier).

    Returns (None, 1.0) for entries that cannot be mapped to a stress score
    (withdrawn, N/A, header rows).

    Multipliers (applied to CDR-anchored base stress):
        Issuer Not Cooperating  → ×3.0  (no monitoring → materially higher risk)
        Watch Negative          → ×1.5
        Watch Positive          → ×0.8
    """
    if not raw:
    ...
    s = raw.strip()
    if s.lower().startswith("crisil "):
        s = s[7:]

    s_lower = s.lower()
    multiplier = 1.0
    if "issuer not cooperating" in s_lower:
        multiplier = 3.0
    if "watch negative" in s_lower:
        multiplier *= 1.5
    elif "watch positive" in s_lower:
        multiplier *= 0.8

    # Base grade is everything before the first "/" or whitespace
    base = re.split(r"[/\s]", s)[0].strip().upper()

    if base.lower() in _SKIP_BASES:
        return None, 1.0

    stress = _LT_STRESS.get(base) if base in _LT_STRESS else _ST_STRESS.get(base)
    return stress, multiplier


def _compute_crisil_stress(bank_facilities: List[Dict]) -> Optional[Dict]:
    """
    Compute a CRISIL-based stress score [0–1] for a company using an
    amount-weighted median across its bank facilities.

    Facilities are sorted largest-to-smallest by amount; the stress score at
    the 50th percentile of cumulative weight is taken as the final score.
    This ensures the largest exposure dominates, not a tail of tiny facilities.

    Returns a dict (to be stored under company.crisilStressScore) or None if
    no facilities carry a scoreable rating.
    """
    scored: List[tuple] = []   # (stress_value, amount_crore, raw_rating_str)
    skipped = 0

    for fac in (bank_facilities or []):
        raw_rating = str(fac.get("rating") or "").strip()
        base_stress, multiplier = _parse_crisil_rating(raw_rating)

        if base_stress is None:
            skipped += 1
            continue

        stress = min(base_stress * multiplier, 1.0)
        try:
            amount = float(fac.get("amount") or 0)
        except (TypeError, ValueError):
            amount = 0.0

        scored.append((stress, amount, raw_rating))

    if not scored:
        return None

    total_amount = sum(a for _, a, _ in scored)

    if total_amount > 0:
        # Amount-weighted median: walk cumulative weight descending by amount
        scored_sorted = sorted(scored, key=lambda x: x[1], reverse=True)
        cumulative = 0.0
        median_stress   = scored_sorted[-1][0]
        dominant_rating = scored_sorted[-1][2]
        for stress, amount, raw_rating in scored_sorted:
            cumulative += amount / total_amount
            if cumulative >= 0.50:
                median_stress   = stress
                dominant_rating = raw_rating
                break
        basis = "weighted_median"
    else:
        # No amounts — plain median on sorted stress values
        stresses = sorted(s for s, _, _ in scored)
        mid = len(stresses) // 2
        if len(stresses) % 2 == 1:
            median_stress = stresses[mid]
        else:
            median_stress = (stresses[mid - 1] + stresses[mid]) / 2
        dominant_rating = ""
        basis = "unweighted_median"

    return {
        "crisilStressScore":   round(median_stress, 6),
        "ratingBasis":         basis,
        "facilitiesScored":    len(scored),
        "facilitiesSkipped":   skipped,
        "dominantRating":      dominant_rating,
        "totalFacilityAmount": round(total_amount, 4),
...
def run(dry_run: bool = False, skip_shp: bool = False) -> None:
    ...
    for rec in records:
        ...
        # ---- CRISIL Stress Score ----
        # Prefer freshly fetched bankFacilities (source 1) over whatever is
        # already on the skeleton record.
        facilities = update.get("bankFacilities") or rec.get("bankFacilities") or []
        stress_result = _compute_crisil_stress(facilities)
        if stress_result:
            update["crisilStressScore"] = stress_result
