...
Our answer is a deliberate and instructive proof of the concept. Constructing a network of 2,858 entities from CRISIL's FY2024–25 rated universe of 9,911 entities integrating credit ratings, MCA filings, Basel III Pillar 3 disclosures, and FinBERT news sentiment. We find that the framework successfully identifies structural hub banks, non-bank bridge intermediaries, and multi-hop contagion pathways at the core of the observable network. At the same time, our results confirm that public disclosures, as currently mandated, are too fragmented to constitute a complete credit map. The lender-facility layer of our heterogenous dataset was able to capture approximately 10% of aggregate bank advances, and 2.86% of corporate nodes carry the required loan-edge structure for full contagion tracing.

The paper's contribution is therefore twofold. First, it demonstrates that a Knowledge Graph is the correct architectural choice for this problem, capable of unifying heterogeneous relational structures that balance-sheet or correlation-based methods cannot represent. Second, it establishes, with precision, exactly what data regulators would need to mandate in order to make such a framework operationally complete. The implications extend to regulators seeking structural surveillance tools, bank risk teams whose ECL models under Ind AS 109 miss network-level concentrations, and institutional investors who require independent risk signals that do not depend on rating agency review cycles.
...
Without access to CRILC or any comparable bilateral credit registry, any network mapping of the Indian financial system must reverse engineer credit exposures from public sources. This paper accepts that constraint as given and asks what, exactly, can be learned within it. The sparsity findings in our results post filtering is a 2.86% loan-edge coverage rate at the corporate level and approximately 10% of aggregate bank advances mapped. These are not a failure of the method; they are the method's most important finding. They locate, with precision, the information gap that regulatory intervention would need to close.
...
The present framework operates on public disclosures. Extending it to sensitive credit data, internal loan books, real-time delinquency trends, covenant breach alerts, requires a privacy-preserving architecture that respects regulatory and competitive constraints while preserving the multi-hop relational structure on which graph-based contagion analysis depends.

Federated Graph Learning (FGL) offers that architecture (He et al., 2021; Zhang et al., 2021). Under FGL, participating institutions collaboratively train graph neural networks on their local subgraph partitions. Each institution, say a bank holding its own borrower-exposure neighborhood, computes structural embeddings locally and exchanges only encrypted neighborhood summaries with a central coordinator. The coordinator aggregates these summaries to reconstruct the cross-institutional topology required for multi-hop contagion tracing, without any participant revealing raw exposure data. Zhang et al. (2021) demonstrated that even when subgraph partitions are missing neighbor nodes across institutional boundaries, missing-neighbor generation techniques can recover the structural information needed for accurate graph-level inference. That property maps directly onto the Indian financial system's regulatory structure, where each bank observes its own CRILC submissions but not those of other banks, and where contagion pathways routinely cross institutional boundaries through shared corporate borrowers.

The viable roadmap for Indian financial stability surveillance is therefore: (1) RBI mandates structured, graph-compatible reporting from banks and systemically important NBFCs, anchored to CRILC's existing bilateral credit data; (2) a regulatory technology consortium implements FGL-based aggregation over the resulting institutional subgraphs, applying differential privacy at the embedding level to prevent reconstruction of individual exposure relationships; (3) the resulting system-wide topology is made available to macro-prudential surveillance functions without exposing any single institution's data. This paper's proof of concept demonstrates that the architectural foundation, Knowledge Graphs as the organizing structure, is correct. What remains is the data mandate and the federated infrastructure to operate it at national scale.
...
24. He, C., Balasubramanian, K., Ceyani, E., Yang, C., Feng, H., Avestimehr, S., & Yu, P. (2021). FedGraphNN: A federated learning system and benchmark for graph neural networks. *arXiv preprint* arXiv:2104.07145.

25. Zhang, K., Yang, C., Li, X., Sun, L., & Yiu, S. M. (2021). Subgraph federated learning with missing neighbor generation. *Advances in Neural Information Processing Systems (NeurIPS), 34,* 6671–6682.
...
10. ✓ **Standard FL/Flower/IBM FL/PP-GNN removed**: Future Directions now leads directly with FGL as the natural extension of the graph-native architecture. No mention of what doesn't work; only what does.
...
12. ✓ **New references added**: He et al. (2021) for FedGraphNN; Zhang et al. (2021) for Subgraph FL with NeurIPS citation. McMahan and Yang references removed (no longer cited).
13. ✓ **Em-dash reduction**: Replaced gratuitous em-dashes with commas throughout Section 12 for more natural prose.
14. ✓ **Three constituencies compressed**: Full stakeholder paragraph removed from Introduction; reduced to one sentence fused with contribution paragraph.
