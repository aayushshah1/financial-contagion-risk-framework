...
ON CREATE SET
    ...
    c.crisilStressScore           = row.crisilStressScore
ON MATCH SET
    ...
    c.crisilStressScore           = CASE WHEN row.crisilStressScore IS NOT NULL THEN row.crisilStressScore ELSE c.crisilStressScore END
"""
...
def build_company_nodes(
    ...
    for doc in company_docs:
        ...
        # crisilStressScore is stored in Mongo as a nested dict
        # {crisilStressScore: float, ratingBasis: str, ...}; extract the scalar.
        stress_doc = doc.get("crisilStressScore")
        crisil_stress: float | None = None
        if isinstance(stress_doc, dict):
            try:
                crisil_stress = float(stress_doc["crisilStressScore"])
            except (KeyError, TypeError, ValueError):
                pass
        elif isinstance(stress_doc, (int, float)):
            crisil_stress = float(stress_doc)
        ...
        records.append({
            ...
            "crisilStressScore":          crisil_stress,
        })
