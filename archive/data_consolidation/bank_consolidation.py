...
def consolidate_bank_data(bank_symbol: str, push_to_db: bool = True) -> dict:
    ...
    try:
        ...
        # Summarise what was extracted — pattern can be Arelle or ET format
        pattern = shareholding.get("shareholdingPattern", {})
        total_shares = shareholding.get("totalShares")
        total_shareholders = shareholding.get("totalShareholders")
        # Arelle format: pattern has an "entities" dict of axis → list
        # ET format    : pattern has nested dicts with "entities" lists
        if "entities" in pattern and isinstance(pattern["entities"], dict):
            entity_count = sum(len(v) for v in pattern["entities"].values())
            axes_count   = len([k for k, v in pattern["entities"].items() if v])
            print(f"  ✓ Extracted shareholding pattern (Arelle) — "
                  f"{entity_count} entities across {axes_count} shareholder axes")
        else:
            # ET path — count non-empty entity lists anywhere in the nested pattern
            import re as _re
            def _count_et_entities(node):
                if isinstance(node, dict):
                    if "entities" in node and isinstance(node["entities"], list):
                        return len(node["entities"])
                    return sum(_count_et_entities(v) for v in node.values())
                return 0
            entity_count = _count_et_entities(pattern)
            print(f"  ✓ Extracted shareholding pattern (ET) — {entity_count} individual shareholder entries")

        if total_shares:
            print(f"  ✓ Total shares: {total_shares:,.0f}  |  Shareholders: {total_shareholders:,.0f}" if total_shareholders else f"  ✓ Total shares: {total_shares:,.0f}")
    except Exception as e:
        print(f"  ✗ Error: {e}")
...
def main():
    ...
    parser.add_argument('--stress-only', action='store_true', help='Run only the stress scoring pipeline (Tasks 10 → 11), skip per-bank consolidation')
    
    args = parser.parse_args()

    # --stress-only: skip all per-bank work and run Tasks 10 → 11 immediately
    if args.stress_only:
