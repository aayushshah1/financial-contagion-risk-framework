...
# ---------------------------------------------------------------------------
# Neo4j connection with HTTPS fallback
# ---------------------------------------------------------------------------

def _to_bolt_fallback_uri(uri: str) -> str | None:
    """
    Derive a direct-bolt fallback URI from a routing neo4j URI.

    The Neo4j Python driver does NOT support https:// — supported schemes are:
        bolt, bolt+s, bolt+ssc, neo4j, neo4j+s, neo4j+ssc

    When a neo4j:// (routing) URI fails (e.g. AuraDB routing lookup error),
    fall back to a direct bolt:// connection on the same host/TLS setting:

        neo4j://host[:port]    ->  bolt://host:7687
        neo4j+s://host[:port]  ->  bolt+s://host:7687   (TLS, AuraDB)
        neo4j+ssc://host[:port]->  bolt+ssc://host:7687 (TLS, self-signed)
        bolt*                  ->  None  (already direct; nothing to fall back to)
    """
    m = re.match(
        r'^(neo4j)(\+s(?:sc)?)?://([^/:]+)(?::\d+)?$',
        uri,
    ...
    if not m:
        return None
    tls_suffix = (m.group(2) or "").lower()   # e.g. "+s", "+ssc", or ""
    host = m.group(3)
    return f"bolt{tls_suffix}://{host}:7687"
...
def get_driver_with_fallback():
    ...
    1. Attempts the primary URI from config (typically neo4j+s:// for AuraDB).
    2. If routing/connectivity verification fails, retries with a direct
       bolt+s:// URI on port 7687 (bypasses the routing lookup).
    Raises ConnectionError if both attempts fail.
    """
    ...
    try:
        ...
        print(f"[neo4j] Connected via primary URI: {NEO4J_URI}")
        return driver
    except Exception as exc:
        primary_err = exc
        print(f"[neo4j] Primary connection failed: {exc}")
        print("[neo4j] Attempting direct-bolt fallback …")

    # ── Direct-bolt fallback ─────────────────────────────────────────────────
    bolt_uri = _to_bolt_fallback_uri(NEO4J_URI)
    if bolt_uri is None:
        raise ConnectionError(
            f"Primary Neo4j connection failed and no bolt fallback could be "
            f"derived from URI '{NEO4J_URI}'.\nCause: {primary_err}"
        )

    try:
        driver = GraphDatabase.driver(bolt_uri, auth=(NEO4J_USER, NEO4J_PASSWORD))
        driver.verify_connectivity()
        print(f"[neo4j] Connected via direct-bolt fallback: {bolt_uri}")
        return driver
    except Exception as fallback_err:
        raise ConnectionError(
            f"Both Neo4j connection attempts failed.\n"
            f"  Primary     ({NEO4J_URI}): {primary_err}\n"
            f"  Bolt direct ({bolt_uri}): {fallback_err}"
        ) from fallback_err
