...
def match_bank_from_crisil_name(lender_name):
    ...
    Uses longest-match strategy so that a more specific variation (e.g.
    "State Bank of India") always beats a shorter substring that happens to
    appear inside it (e.g. "Bank of India" inside "State Bank of India").
    Returns bank symbol if matched, None otherwise.
    """
    if not lender_name or lender_name.lower() in ['not applicable', 'na', 'null']:
        return None

    ln_lower = lender_name.lower()
    best_symbol = None
    best_len = 0
